      ...
      <div >
       ...
       <Banner title='Super-Admin Dashboard '  />
        ...
        <section className="py-12 px-20 bg-white">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">{superAdminTools.map((tool, index) => (
              <div key={index} className="p-6 bg-blue-50 rounded-lg shadow-lg flex flex-col items-center text-center">
                <h3 className="text-xl font-bold text-blue-950">{tool.title}</h3>
                <p className="text-gray-600 text-sm mt-2">{tool.description}</p>
                <a href={tool.link} className="mt-4 px-4 py-2 bg-blue-900 text-white rounded-lg hover:bg-blue-700 transition">{tool.buttonLabel}</a>
              </div>))}
          </div>
        </section>
      </div>
